...
@Component
public class ProtaxImportDispatcher {

    private final Logger logger = LoggerFactory.getLogger(ProtaxImportDispatcher.class);
    ...
    @Autowired
    @Qualifier("protaxImportDispatchDestination")
    Destination protaxImportDestination;
    ...
    public void send(final String message, String jobId) {
        ...
        try {
            jmsTemplate.send(protaxImportDestination, new MessageCreator() {
                public Message createMessage(Session session) throws JMSException {
                    ...
                    logger.info("Sending message to Protax Import Queue: " + textMessage);

                    return textMessage;
                }
            });
        } catch (JmsException e) {
            logger.error("Exception while dispatching message to Canada Protax queue for jobId={} destination={}", jobId, protaxImportDestination.toString());
            throw e;
        }
    }
}
